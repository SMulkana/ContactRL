"""
Author: Sundas Mulkana
Date: 2025-10-01

ContactRL — Safe-Contact Reaching Environment (Gymnasium + PyBullet)

This module defines a custom Gymnasium environment for a UR3e robotic arm with a Robotiq gripper,
simulated in PyBullet. The agent learns to move the end-effector toward a human hand to pick an object
while respecting safety and smoothness: rewards combine reaching accuracy, contact safety
(based on contact force thresholds), motion smoothness (jerk-like penalty), proximity shaping,
and simple time efficiency. The env implements reset/step/render, computes kinematics via IK,
tracks episode stats, and can capture frames for later analysis.
...
class ContactRL(gym.Env):
    ...
    def step(self, action):
        ...
        F_N_hand_desirable_max = 50 #N maximumm allowable force on the hand to lift object without causing causing discomfort
        ...
        #orn = p.getLinkState(self.robot_id, self.ee_link_idx)[1]
...
# ─── 3.  proximity: penalise large steps relative to max reach distance ─
        ...
        if contact_gripper_objectB:
            ...
            if finger_object_collision:
                 for point in contact_gripper_objectB:
                     ...
                     reach_bonus = 100
                     if self.forceB <= F_N_hand_desirable_max:
        ...
        reward = reward_reach + 1*reward_smooth + 1*Reward_proximity + 2*reward_safe  + reach_bonus + 1*reward_time
  ...
  | p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS
        ...
        objectB = [0.3,0.1, 0.05]
        objectA = [0.3,0.1, 0.07]
        orientation_B = get_quaternion_90_x()
        self.objectBUid = p.loadURDF("/path/to/folder/env/urdf/objectB.urdf", basePosition=objectB, baseOrientation=orientation_B, useFixedBase=True)
        self.objectAUid = p.loadURDF("/path/to/folder/env/urdf/objectA.urdf", basePosition=objectA, baseOrientation=orientation_B, useFixedBase=True)
